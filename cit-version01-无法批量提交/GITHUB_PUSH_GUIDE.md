...
cit-version01-无法批量提交 add test.txt

# 3. 提交更改
cit-version01-无法批量提交 commit -m "feat: 添加测试文件"

# 4. 添加远程仓库
cit-version01-无法批量提交 remote add origin https://github.com/code-100-precent/cit.git

# 5. 推送到GitHub (真实推送)
cit-version01-无法批量提交 push --github-token YOUR_TOKEN origin main
```

### 2. **命令参数说明**
```bash
cit-version01-无法批量提交 push [远程名] [分支名] --github-token [令牌]
...
# SSH格式 (需要SSH密钥配置)
cit-version01-无法批量提交 remote add origin git@github.com:code-100-precent/cit-version01-无法批量提交.git
```
...
```bash
# 启用详细输出
cit-version01-无法批量提交 push --github-token TOKEN origin main -v

# 检查远程配置
cit-version01-无法批量提交 remote list
...
cit-version01-无法批量提交 commit -m "feat: 新功能"
cit-version01-无法批量提交 push --github-token TOKEN origin main
```

### 2. **教学演示**
```bash
# 展示真实的Git工作流
cit-version01-无法批量提交 push origin main          # 模拟推送
cit-version01-无法批量提交 push --github-token TOKEN origin main  # 真实推送
```
...
cit-version01-无法批量提交 push --github-token $GITHUB_TOKEN origin main
```
